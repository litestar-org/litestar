...
class PersonRepository(AbstractAsyncRepository[VanillaDataClassPerson]):
    ...
    async def add_many(self, data: list[VanillaDataClassPerson]) -> list[VanillaDataClassPerson]:
        return [VanillaDataClassPersonFactory.build(**asdict(datum)) for datum in data]

    async def add(self, data: VanillaDataClassPerson) -> VanillaDataClassPerson:
    ...
    async def update(self, data: VanillaDataClassPerson) -> VanillaDataClassPerson:
        return VanillaDataClassPersonFactory.build(**asdict(data))

    async def update_many(self, data: list[VanillaDataClassPerson]) -> list[VanillaDataClassPerson]:
        return [VanillaDataClassPersonFactory.build(**asdict(datum)) for datum in data]
...
class TestGenericController(GenericController[VanillaDataClassPerson]):
    ...
    # fix: allow lambdas without arguement
    instance_id_factory = lambda x: str(uuid.uuid4())
...
def test_create_instance() -> None:
    with create_test_client(TestGenericController) as client:
        ...
        assert VanillaDataClassPerson(**response.json()).last_name == instance.last_name
