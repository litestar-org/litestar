...
def test_classic_pagination_data_shape(paginator: Any) -> None:
    ...
    async def async_handler(
        page_size: FromQuery[int], current_page: FromQuery[int]
    ) -> ClassicPagination[DataclassPerson]:
        return await paginator(page_size=page_size, current_page=current_page)  # type: ignore[no-any-return]

    @get("/sync")
    def sync_handler(page_size: FromQuery[int], current_page: FromQuery[int]) -> ClassicPagination[DataclassPerson]:
...
def test_limit_offset_pagination_data_shape(paginator: Any) -> None:
    @get("/async")
    async def async_handler(limit: FromQuery[int], offset: FromQuery[int]) -> OffsetPagination[DataclassPerson]:
        return await paginator(limit=limit, offset=offset)  # type: ignore[no-any-return]

    @get("/sync")
    def sync_handler(limit: FromQuery[int], offset: FromQuery[int]) -> OffsetPagination[DataclassPerson]:
...
def test_cursor_pagination_data_shape(paginator: Any) -> None:
    @get("/async")
    async def async_handler(cursor: FromQuery[Optional[str]] = None) -> CursorPagination[str, DataclassPerson]:
        return await paginator(cursor=cursor, results_per_page=5)  # type: ignore[no-any-return]

    @get("/sync")
    def sync_handler(cursor: FromQuery[Optional[str]] = None) -> CursorPagination[str, DataclassPerson]:
