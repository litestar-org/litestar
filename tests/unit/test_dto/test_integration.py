...
from . import Model, TestModelDataDTO, TestModelReturnDTO


def test_dto_defined_on_handler() -> None:
    @post(dto=TestModelDataDTO, signature_namespace={"Model": Model})
...
def test_dto_defined_on_router() -> None:
    ...
    router = Router(path="/", route_handlers=[handler], dto=TestModelDataDTO)
...
def test_dto_defined_on_app() -> None:
    ...
    with create_test_client(route_handlers=handler, dto=TestModelDataDTO) as client:
        response = client.post("/", json={"what": "ever"})
        assert response.status_code == 201
        assert response.json() == {"a": 1, "b": "2"}
...
def test_set_dto_none_disables_inherited_dto() -> None:
    ...
    mock_dto = MagicMock(spec=TestModelDataDTO)
...
def test_dto_and_return_dto() -> None:
    ...
    with create_test_client(route_handlers=handler, dto=TestModelDataDTO, return_dto=TestModelReturnDTO) as client:
        response = client.post("/", json={"what": "ever"})
        assert response.status_code == 201
        assert response.json() == {"a": 1, "b": "2"}
