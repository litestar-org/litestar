...
from starlite.middleware.session.sqlalchemy_backend import AsyncSQLAlchemyBackend, SQLAlchemyBackend
...
async def test_get_set(server_side_backend, session_data) -> None:
...
async def test_get_set_multiple_returns_correct_identity(server_side_backend) -> None:
...
async def test_delete(server_side_backend) -> None:
...
async def test_delete_idempotence(server_side_backend, session_data) -> None:
    await server_side_backend.set("foo", session_data)
...
async def test_delete_all(server_side_backend) -> None:
...
async def test_max_age_expires(server_side_backend: ServerSideBackend, session_data) -> None:
...
async def test_file_backend_init_directory(file_session_backend: FileBackend, session_data) -> None:
    shutil.rmtree(file_session_backend.path)
    await file_session_backend.set("foo", session_data)


async def test_file_backend_path(file_session_backend: FileBackend, session_data) -> None:
    await file_session_backend.set("foo", session_data)

    assert await (file_session_backend.path / "foo").exists()


async def test_load_file_not_found_returns_emtpy_session(file_session_backend: FileBackend, session_data) -> None:
    await file_session_backend.set("foo", session_data)
...
async def test_sqlalchemy_backend_delete_expired(sqlalchemy_session_backend) -> None:
...
async def test_sqlalchemy_async_backend_delete_expired(async_sqlalchemy_session_backend) -> None:
...
def test_sqlalchemy_config_dynamic_backend(sqlalchemy_backend_config, async_sqlalchemy_backend_config) -> None:
