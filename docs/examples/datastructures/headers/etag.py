...
class MyController(Controller):
    ...
    @get("/timestamp", etag=ETag(value="bar", weak=True), sync_to_thread=False)
...
@get("/population-dynamic", etag=ETag(documentation_only=True), sync_to_thread=False)
def get_population_count_dynamic() -> Response[str]:
    ...
    return Response(
        ...
        headers={"etag": f'"{population_count}"'},
...
app = Litestar(
    route_handlers=[MyController, get_population_count, get_population_count_dynamic], etag=ETag(value="bar")
)
