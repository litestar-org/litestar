...
class CustomOperation(Operation):
    ...
    x_code_samples: Optional[List[Dict[str, str]]] = field(
        default=None, metadata={"alias": "x-codeSamples"}
    )
...
async def route(request: Request) -> str:
    ...
    if (await request.body()) == b"OK":
        return "OK"
    raise HTTPException(
        status_code=HTTP_400_BAD_REQUEST, detail="request payload must be OK"
    )
