...
# model and the ASGI connection and returns the `User` instance correlating to
# it.
#
# Notes:
...
def some_route_handler(request: "Request[User, Token, Any]") -> Any:
    ...
    # from the data encoded in the authorization header
...
# We initialize the app instance and pass the `jwt_auth.on_app_init` handler to
# the constructor. The hook handler will inject the JWT middleware and OpenAPI
# configuration into the app.
app = Litestar(
