...
# Let's assume we have an `User` model that is a Pydantic model. This is not
...
# token model and the ASGI connection and returns the `User` instance
# correlating to it.
#
# Notes:
...
# handler to the constructor. The hook handler will inject the JWT middleware
# and OpenAPI configuration into the app.
app = Litestar(
