...
collection should have an empty ``children`` collection.

The reason they do not is due to :attr:`max_nested_depth <litestar.dto.factory.DTOConfig.max_nested_depth>` and its
default value of ``1``. The ``max_nested_depth`` attribute is used to limit the depth of nested objects that are
