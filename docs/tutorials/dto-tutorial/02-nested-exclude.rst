...
To demonstrate, let us adjust our script to add an ``Address`` model, that is related to the ``Person`` model:
...
The ``Address`` model has three attributes, ``street``, ``city``, and ``country``, and we have added an ``address``
