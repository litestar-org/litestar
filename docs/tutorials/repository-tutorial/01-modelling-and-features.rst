...
.. literalinclude:: /examples/contrib/sqlalchemy/sqlalchemy_declarative_models.py
    :language: python
    :caption: ``app.py``
    :lines: 10-11, 18-20, 23-26
    :linenos:

The book entity is not considered a "strong" entity and therefore always requires an
author to be created. We need to configure our SQLAlchemy classes so that they are aware
...
that the only modification here is the parent class from which we inherit. This
minor change endows the table ``book`` with automatic timestamp columns
(``created`` and ``updated``) upon deployment!

.. note::

    If your application requires integer-based primary keys, equivalent base
    model and base audit model implementations can be found at
...
.. dropdown:: Full Code (click to toggle)
    ...
    .. literalinclude:: /examples/contrib/sqlalchemy/sqlalchemy_declarative_models.py
        :language: python
        :caption: ``app.py``
        :emphasize-lines: 9, 17, 22-29, 36-44
        :linenos:
