...
cases where you need to add in additional functionality. Let us explore ways that we
...
primary keys in the URL.  While UUIDs do not have this same problem, they are not
...
First, we will create a ``SlugKey`` field mixin that adds a text-based, URL-friendly,
...
Since the ``BlogPost.title`` field is not marked as unique, this means that we will have
...
We are all set to use this in our routes now.  First, we will convert our incoming
Pydantic model to a dictionary.  Next, we will fetch a unique slug for our text.
