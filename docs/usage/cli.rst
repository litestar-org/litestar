...
The CLI is not installed by default. Its dependencies are bundled in the ``cli`` extra, which additionally
provides |uvicorn|_ (needed to run your application using ``litestar run``) and
`jsbeautifier <https://pypi.org/project/jsbeautifier/>`_ (used to beautify the Typescript generated by the
``litestar schema typescript`` command). They can be installed independently, but we recommend installing the
``standard`` extra which conveniently bundles the CLI along with other commonly used optional dependencies.
