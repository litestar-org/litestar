...
.. literalinclude:: /examples/request_data/request_data_1.py
    :caption: Parsing request data into a dictionary
...
.. literalinclude:: /examples/request_data/request_data_2.py
    :caption: Parsing request data into a :func:`dataclass <dataclasses.dataclass>`
...
.. literalinclude:: /examples/request_data/request_data_3.py
    :caption: Customizing request body validation and OpenAPI documentation

Content-type
------------

By default, Litestar will try to parse the request body as JSON. While this may be desired
in most cases, you might want to specify a different type.
You can do so by passing a :class:`RequestEncodingType <litestar.enums.RequestEncodingType>` to
:class:`Body <litestar.params.Body>`.
This will also help to generate the correct media-type in the OpenAPI schema.

URL Encoded Form Data
^^^^^^^^^^^^^^^^^^^^^

To access data sent as `url-encoded form data <https://developer.mozilla.org/en-US/docs/Web/HTTP/Methods/POST>`_,
i.e. ``application/x-www-form-urlencoded`` ``Content-Type`` header, use :class:`Body <litestar.params.Body>` and
specify :class:`RequestEncodingType.URL_ENCODED <litestar.enums.RequestEncodingType>` as the
:paramref:`~litestar.params.Body.params.media_type`:

.. literalinclude:: /examples/request_data/request_data_4.py
    :caption: Parsing URL-encoded form data

.. note:: URL encoded data is inherently less versatile than JSON data - for example, it cannot handle complex
...
.. literalinclude:: /examples/request_data/request_data_5.py
    :caption: Parsing multipart form data
...
.. tab-set::
    ...
    .. tab-item:: Async
        ...
        .. literalinclude:: /examples/request_data/request_data_6.py
            :caption: Accessing a single uploaded file asynchronously

    .. tab-item:: Sync

        .. literalinclude:: /examples/request_data/request_data_7.py
            :caption: Accessing a single uploaded file synchronously
...
.. literalinclude:: /examples/request_data/request_data_8.py
    :caption: Handling Multiple File Uploads with Pydantic Models

Files as a dictionary
^^^^^^^^^^^^^^^^^^^^^

If you do not care about parsing and validation and only want to access the form data as a dictionary,
you can use a :term:`dict <python:dictionary>` instead:

.. literalinclude:: /examples/request_data/request_data_9.py
    :caption: Accessing Uploaded Files as a Dictionary
...
.. literalinclude:: /examples/request_data/request_data_10.py
    :caption: Handling File Uploads as a List without Filenames

MessagePack data
----------------

To receive `MessagePack <https://msgpack.org/>`_ data, specify the appropriate ``Content-Type``
for :class:`Body <litestar.params.Body>` , by using
:class:`RequestEncodingType.MESSAGEPACK <.enums.RequestEncodingType>`:

.. literalinclude:: /examples/request_data/msgpack_request.py
   :caption: Receiving and Handling MessagePack Data
...
Litestar supports custom :paramref:`~litestar.app.Litestar.request_class` instances, which can be used to
further configure the default :class:`Request`. The example below illustrates how to implement custom
request class for the whole application.

.. dropdown:: Example of a custom request at the application level

    .. literalinclude:: /examples/request_data/custom_request.py
        :caption: Implementing a Custom Request Class at the Application Level
