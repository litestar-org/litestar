...
class OpenAPIPlugin(InitPluginProtocol, ReceiveRoutePlugin):
    ...
    def provide_openapi_schema_json(self, request: Request[Any, Any, Any]) -> bytes:
        if not self._openapi_schema_json:
            self._openapi_schema_json = encode_json(
                self.provide_openapi().to_schema(), get_serializer(request.app.type_encoders)
            )
        return self._openapi_schema_json

    def provide_openapi_schema_yaml(self, request: Request[Any, Any, Any]) -> bytes:
        if not self._openapi_schema_yaml:
            ...
            schema = msgspec.to_builtins(
                self.provide_openapi().to_schema(), enc_hook=get_serializer(request.app.type_encoders)
            )
            self._openapi_schema_yaml = dump_yaml(schema, default_flow_style=False).encode("utf-8")
