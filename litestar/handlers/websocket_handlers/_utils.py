...
class ListenerContext:
    ...
    handle_receive: Callable[[WebSocket, AbstractDTO | None], Any]
    handle_send: Callable[[WebSocket, Any, AbstractDTO | None], Coroutine[None, None, None]]
    json_encoder: JsonEncoder
    listener_callback: AnyCallable
    listener_callback_signature: ParsedSignature
    pass_socket: bool
    resolved_data_dto: type[AbstractDTO] | None
    resolved_return_dto: type[AbstractDTO] | None
...
) -> Callable[[WebSocket, AbstractDTO | None], Coroutine[Any, None, None]]:
    if resolved_data_dto:

        async def handle_receive(socket: WebSocket, dto: AbstractDTO | None) -> Any:
            received_data = await socket.receive_data(mode=receive_mode)
            if isinstance(received_data, str):
                received_data = received_data.encode("utf-8")
            return cast("AbstractDTO", dto).decode(received_data)
...
) -> Callable[[WebSocket, Any, AbstractDTO | None], Coroutine[None, None, None]]:
    if resolved_return_dto:

        async def handle_send(socket: WebSocket, data_to_send: Any, dto: AbstractDTO | None) -> None:
            data_to_send = json_encoder.encode(cast("AbstractDTO", dto).encode(data_to_send))
...
def create_handler_function(
    ...
    lifespan_manager: Callable[..., AbstractAsyncContextManager[Any]],
