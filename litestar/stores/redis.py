...
class RedisStore(NamespacedStore):
    ...
    async def set(
        self, key: str, value: str | bytes, expires_in: int | timedelta | None = None, keep_ttl: bool = False
    ) -> None:
        """Set a value.
        ...
        Raises:
            ImproperlyConfiguredException: If both ``expires_in`` and ``keep_ttl`` are set, as these options are mutually exclusive
        ...
        if expires_in is not None and keep_ttl:
            raise ImproperlyConfiguredException(
                "Cannot set both 'expires_in' and 'keep_ttl': these options are mutually exclusive"
            )
