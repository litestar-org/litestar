...
# litestar/contrib/mongodb_motor/repository/_async.py
...
from litestar.contrib.mongodb_motor.repository._util import (
    _convert_cursor_to_list_sync,
    wrap_pymongo_exception,
)
from litestar.contrib.repository import (
...
class MongoDbSyncRepository(AbstractSyncRepository[DocumentType]):
    """Motor based implementation of the repository interface."""

    id_attribute = "_id"
    model_type: type[DocumentType] = dict

    def __init__(self, collection: Collection, **kwargs: Any) -> None:  # pyright: ignore[reportGeneralTypeIssues]
        super().__init__(**kwargs)
        self.collection = collection

    def add(self, data: DocumentType) -> DocumentType:
    ...
    def add_many(self, data: list[DocumentType]) -> list[DocumentType]:
    ...
    def get(self, item_id: Any, **kwargs: Any) -> DocumentType:
    ...
    def get_or_create(
        ...
        doc = self.get_one_or_none(**match_filter)
    ...
    def get_one_or_none(self, **kwargs: Any) -> DocumentType | None:
        ...
        """
        with wrap_pymongo_exception():
            return cast(Optional[DocumentType], self.collection.find_one(kwargs))

    def update(self, data: DocumentType) -> DocumentType:
    ...
    def upsert(self, data: DocumentType) -> DocumentType:
    ...
    def list_and_count(self, *filters: FilterTypes, **kwargs: Any) -> tuple[list[DocumentType], int]:
        ...
        with wrap_pymongo_exception():
            ...
            docs = self._convert_cursor_to_list(cursor)
            return docs, len(docs)

    def _convert_cursor_to_list(self, cursor: Any) -> list[DocumentType]:
        return _convert_cursor_to_list_sync(cursor)

    def list(self, *filters: FilterTypes, **kwargs: Any) -> list[DocumentType]:
        ...
        with wrap_pymongo_exception():
            cursor = self.collection.find(query)
            return self._convert_cursor_to_list(cursor)
