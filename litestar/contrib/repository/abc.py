...
__all__ = ("AbstractAsyncRepository", "AbstractSyncRepository", "FilterTypes")

T = TypeVar("T")
AsyncRepoT = TypeVar("AsyncRepoT", bound="AbstractAsyncRepository")
SyncRepoT = TypeVar("SyncRepoT", bound="AbstractSyncRepository")
...
class AbstractSyncRepository(Generic[T], metaclass=ABCMeta):
    ...
    def update(self, data: T) -> T:
        ...
        Args:
            data: An instance that should have a value for :attr:`id_attribute <AbstractSyncRepository.id_attribute>` that exists in the
                collection.
    ...
    def update_many(self, data: list[T]) -> list[T]:
        ...
        Args:
            data: A list of instance that should have a value for :attr:`id_attribute <AbstractSyncRepository.id_attribute>` that exists in the
                collection.
    ...
    def upsert(self, data: T) -> T:
        ...
        Args:
            data: Instance to update existing, or be created. Identifier used to determine if an
                existing instance exists is the value of an attribute on ``data`` named as value of
                :attr:`id_attribute <AbstractSyncRepository.id_attribute>`.
    ...
    def filter_collection_by_kwargs(self, collection: CollectionT, /, **kwargs: Any) -> CollectionT:
        ...
        Raises:
            RepositoryError: if a named attribute doesn't exist on :attr:`model_type <AbstractSyncRepository.model_type>`.
        """
    ...
    @classmethod
    def get_id_attribute_value(cls, item: T | type[T]) -> Any:
        """Get value of attribute named as :attr:`id_attribute <AbstractSyncRepository.id_attribute>` on ``item``.

        Args:
            item: Anything that should have an attribute named as :attr:`id_attribute <AbstractSyncRepository.id_attribute>` value.

        Returns:
            The value of attribute on ``item`` named as :attr:`id_attribute <AbstractSyncRepository.id_attribute>`.
        """
        return getattr(item, cls.id_attribute)
    ...
    def set_id_attribute_value(cls, item_id: Any, item: T) -> T:
        ...
        Returns:
            Item with ``item_id`` set to :attr:`id_attribute <AbstractSyncRepository.id_attribute>`
        """
        setattr(item, cls.id_attribute, item_id)
        return item
