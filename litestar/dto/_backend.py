...
def _annotate_type_with_metadata(field_type: Any, metadata: Any) -> Any:
    """Apply ``metadata`` to ``field_type``, taking care to preserve ``Optional`` types.

    msgspec only allows constraints such as ``min_length`` to be set on the constrained type itself, not on a
    ``Union`` that contains it (e.g. ``Optional[str]``). So for a field type like ``Optional[str]``, the metadata
    must be applied to the ``str`` member of the union, rather than to the union as a whole.
    """
    if get_origin(field_type) is Union:
        args = get_args(field_type)
        non_none_args = tuple(arg for arg in args if arg is not NoneType)
        if len(non_none_args) != len(args):
            return Union[*(Annotated[arg, metadata] for arg in non_none_args), NoneType]
    ...
    for field_definition in field_definitions:
        ...
        if field_definition.passthrough_constraints:
            if (field_meta := _create_struct_field_meta_for_field_definition(field_definition)) is not None:
                field_type = _annotate_type_with_metadata(field_type, field_meta)
        elif field_definition.kwarg_definition:
            field_type = _annotate_type_with_metadata(field_type, field_definition.kwarg_definition)
