...
from starlite.asgi.routing_trie.traversal import (
    parse_scope_to_route,
    resolve_handler_node,
)
...
class ASGIRouter:
    ...
    def __init__(
        ...
        self.registered_routes: Set[Union["HTTPRoute", "WebSocketRoute", "ASGIRoute"]] = set()
    ...
    async def __call__(self, scope: "Scope", receive: "Receive", send: "Send") -> None:
        ...
        try:
            asgi_handlers, is_mount = parse_scope_to_route(
                root_node=self.root_route_map_node, scope=scope, plain_routes=self.plain_routes
            )
            asgi_app, handler = resolve_handler_node(scope=scope, asgi_handlers=asgi_handlers, is_mount=is_mount)
    ...
    def construct_routing_trie(self) -> None:
        ...
        for route in new_routes:
            node = map_route_to_trie(
                ...
                plain_routes=self.plain_routes,
            )
